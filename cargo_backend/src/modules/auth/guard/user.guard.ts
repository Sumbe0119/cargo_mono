...
export class UserGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext | ExecutionContextHost): Promise<boolean> {
    try {
      ...
      if (userToken) {
        ...
        if (decodedToken) {
          const user = await this.userService.getUser({username: decodedToken.username});
